...
currPath = "./"
fileName = "graph.txt"
fileLabelName = "label.txt"
fileOutName = "graph.adm"
...
fullFileName = os.path.join(currPath, fileLabelName);
fileGraph = open(fullFileName);
fileLabelContent = fileGraph.readlines();
fileGraph.close();
...
labelNum = len(fileLabelContent);
print labelNum;
dic = {};
for i in range(0, labelNum):
    line = fileLabelContent[i];
    #print "line:"+line;
    line = line.rstrip();
...
for i in range(0, tupleNum):
    ...
    lineOut = lineOut+'"label":"'+dic[lineArray[0]]+'",';
    lineTargetNode = "";
    lineWeight="";
